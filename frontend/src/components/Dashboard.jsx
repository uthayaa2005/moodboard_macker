        ...
        {boards.length === 0 ? (
          ...
          <div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 2xl:grid-cols-5 gap-8">
              {boards.map((board) => (
                <Link to={`/boards/${board._id}`} key={board._id} className="group">
  <div className="bg-gray-800 rounded-2xl shadow-xl hover:shadow-2xl transform hover:scale-[1.02] transition-all duration-300 overflow-hidden border border-gray-700/50 hover:border-gray-600/50 relative">
    {board.image ? (
      <img
        src={board.image}
        alt="Board Cover"
        className="w-full h-56 object-cover group-hover:opacity-40 transition-opacity duration-300"
      />
    ) : (
      <div className="w-full h-56 bg-gradient-to-br from-gray-700 to-gray-800 flex items-center justify-center text-gray-400">
